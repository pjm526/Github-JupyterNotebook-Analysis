...
con = psycopg2.connect(host="ec2-34-196-79-237.compute-1.amazonaws.com",dbname="testing", user="ubuntu", password="password!")
...
app.layout = html.Div([
    html.H1('Jupyter Trends',style={'text-align': 'left', color: 'orange' 'padding': '20px',}),
...
def render_content(tab):
    ...
    lib = lib_list[0]
    qry = "select datetime,SUM(CAST(lib_counts AS int)) as users from mahaveer WHERE library = '" + lib + "' GROUP BY datetime ORDER BY datetime ASC"
    ...
    if tab == 'tab-1':
        return html.Div([
            ...
            dcc.Graph(
                ...
                figure={
                    'data': [
                    {'x': df_lib1['datetime'], 'y': df_lib1['users'], 'type': 'line', 'name': lib_list[0]}],
                    'layout':{
                            'xaxis':{
                                'title':'Date',
                                },
...
def update_graph(lib):
    ...
    return {
        'data': [
        {'x': df_lib1['datetime'], 'y': df_lib1['users'], 'type': 'line', 'name': lib}],
        'layout':{
                'xaxis':{
                    'title':'Date',
                    },
